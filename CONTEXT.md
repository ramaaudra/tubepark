...
- **Watch-page Park & Close Capture** (F10): an always-visible pill button portaled to `<body>`, `position: fixed` top-right of the viewport, scoped to `/watch?v=` + `/shorts/{id}`. Sibling to Hover-to-Park (which park-onlys hovered sidebar cards); this captures the MAIN video — `videoId` from the URL, `title` from `document.title`, `channel` via `resolveWatchPageChannel()`, and `resumeAt` from `readMainVideoCurrentTime()` read **locally at click time** (no `GET_TAB_META` round-trip — the content script IS the reader). One click parks then relays to the background (`PARK_AND_CLOSE_TAB`), which calls `chrome.tabs.remove(sender.tab.id)` on success/duplicate (full → no close + toast). 1-click, no confirm, no undo; the queue + `resumeAt` is the safety net. A subtle yellow dot marks an already-parked video (cosmetic — click always park+close, never unpark). Nol DOM coupling (viewport-fixed); SPA navigation is watched (`yt-navigate-finish` + `popstate` + a 1s `location.href` poll) so the button re-resolves per navigation. See `src/entrypoints/content.ts` `WatchPageParkButton`.
