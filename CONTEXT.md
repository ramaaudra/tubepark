...
- **Pending Removal**: a single slot (`{ operationId, owner, videos, requestedAt, expiresAt }`, 1 or N items) owned by the **background** service worker — never by a UI surface — and persisted in `chrome.storage.local`. Removal targets use each video's `(id, addedAt)` identity. The 5s `setTimeout` is only an accelerator; an alarm and startup reconciliation commit by the absolute `expiresAt` if the worker restarts. Closing the Side Panel mid-window still commits, because the transaction is not owned by the panel.
- **Undo**: a cancel message (`CANCEL_REMOVE`) carrying the matching `operationId` and owner (`popup`, `sidepanel`, or `content`). A stale or different surface cannot cancel a newer operation, and surfaces do not display Undo for another surface's action. Undo writes nothing — the video was never removed from storage — so undo can never fail and never hits the capacity cap. Requesting a second removal while one is pending **commits the first** (it is not silently cancelled).
...
- **Interface**: `getActiveTab()`, `getYouTubeTabs()`, `getWatchTabs()`, `sendMessage(id, message)`, `closeTab(id)`, `openVideo(videoId)`, `openSidePanel()`.
...
- **Two adapters**: `RealTabOperations` (production, wraps `chrome.*`) and `TestTabOperations` (test double with call recording + configurable returns). YouTube-tab discovery and content-script messages used by the background also cross this seam. Per the codebase-design principle: "one adapter = hypothetical seam, two = real."
...
### Capture Mechanisms
- **Context Menu Capture**: Right-click a YouTube video **link** on a YouTube page → "Park This Video". Scoped via `contexts: ["link"]` + `targetUrlPatterns` (watch / youtu.be / shorts) on the link URL and `documentUrlPatterns` for `*://*.youtube.com/*` plus `*://youtu.be/*`, so the menu only appears where a content script is loaded and can actually park — never on blank space, never on YouTube links in other sites (silent-fail closure). See ADR-0001 (+ its G3 Correction). Card-miss fallback reads channel via `resolveWatchPageChannel()` (same as tab-park).
